...
# ---------------------------
# CONFIG - change these if necessary
# ---------------------------
# Local Prolog file (absolute path recommended)
...
def call_prolog_online(goal: str):
    ...
    # We'll send the program as 'src' and the query as 'ask'. SWISH's API supports several fields.
    payload = {
        # 'src' or 'program' naming may differ; provide 'src' and 'program' for compatibility
        "src": prog,
        "ask": goal,
        # We want the textual stdout printed by the program; set options
        "format": "json"
    }

    # If this endpoint changes, we'll try fallback modes below.
    try:
    ...
    # We'll search recursively for strings that look like our expected format: [a,b]|12.00|34.00
...
def find_route_prolog(criteria_atom, start_atom, goal_atom, use_online=False):
    ...
    if not m:
        # maybe returned raw JSON; try to find bracket+bars pattern
...
class PathFinderApp:
    def __init__(self, root):
        ...
        # Local SWIPL path input (if needed)
        ...
        # start / goal selection: we will load nodes from prolog graph
